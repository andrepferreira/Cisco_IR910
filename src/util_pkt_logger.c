...
int main(int argc, char **argv)
{
	...
	while ((quit_sig != 1) && (exit_sig != 1)) {
		...
		for (i=0; i < nb_pkt; ++i) {
			...
			/* Print out the array of payload parts
			*/
			for (int n=0; pay_array[n] != NULL; ++n)
			{
				printf ("Payload [%d] is: %s\n", n, pay_array[n]);
			}


			// Ok, now send the payload parts to the queue
			send_mqtt_string ("10.12.14.68", "gateway", "password", "ir910/lora/name", pay_array[1]);
			send_mqtt_string ("10.12.14.68", "gateway", "password", "ir910/lora/pkt", pay_array[2]);
			send_mqtt_string ("10.12.14.68", "gateway", "password", "ir910/lora/batt", pay_array[3]);
			send_mqtt_string ("10.12.14.68", "gateway", "password", "ir910/lora/temp", pay_array[4]);
